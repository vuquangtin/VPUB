...
namespace sNonResidenComponent.WorkItems.ManageMeeting
{
    public partial class FrmPartakerList : Form
    {
        ...
        List<Partaker> jsonListPartaker;
        ...
        /// <summary>
        /// format json partaker
        /// </summary>
        /// <param name="listPartakerNonresident"></param>
        private void FormatJsonPartaker(string listPartakerNonresident)
        {
            jsonListPartaker = new List<Partaker>();
            jsonListPartaker = JsonConvert.DeserializeObject<List<Partaker>>(listPartakerNonresident);
            int totalRecords = 0;
            currentPageIndex = 1;
            ...
            {
                List<Partaker> result = jsonListPartaker.Skip(skip).Take(take).ToList();
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            if (filePath != null)
            {
                try
                {
                    ...
                    value = "";
                    index++;
                    String lblRoom = MessageValidate.GetMessage(rm, "lblRoom");
                    value = (lblRoom == null ? string.Empty : lblRoom) + " " + (txtRoomname.Text == null ? string.Empty : txtRoomname.Text.ToString());
                    GemboxUtils.Instance.AddCellCustom(index, 0, value == null ? string.Empty : value);
                    value = "";
                    index++;
