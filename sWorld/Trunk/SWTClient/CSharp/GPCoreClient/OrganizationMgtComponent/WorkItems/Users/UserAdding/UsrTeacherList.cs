...
{
    public partial class UsrTeacherList : CommonUserControl, IUserAddingDialog
    {
        #region Properties
        ...
        {
            get { return new object[] { teacherId }; }
        }
        ...
        private void bgwLoadFaculty_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                result = JavaCommunicationPersonalization.Instance.GetFacultyList(sessionId, filter);
            }
        ...
        private void LoadTeachers()
        {
            ...
            {
                TeacherFilterDto filter = new TeacherFilterDto();
        ...
        private void bgwLoadTeacher_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            TeacherFilterDto teacherFilter = (TeacherFilterDto)e.Argument;
            string sessionId = StorageService.CurrentSessionId;
            int totalRecords = 0;

            try
            {
                result = JavaCommunicationPersonalization.Instance.GetTeacherList(sessionId, teacherFilter, (currentPageIndex - 1) * LocalSettings.Instance.RecordsPerPage, LocalSettings.Instance.RecordsPerPage, out totalRecords);
            }
            catch (TimeoutException)
            {
                MessageBoxManager.ShowErrorMessageBox(this, CommonMessages.TimeOutExceptionMessage);
        ...
        private void bgwLoadTeacher_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            List<TeacherDto> result = (List<TeacherDto>)e.Result;
            if (result != null)
            {
                foreach (TeacherDto r in result)
                {
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBoxManager.ShowErrorMessageBox(this, "Bạn chưa chọn giảng viên sẽ được cấp tài khoản!", "Thao Tác Sai");
                return;
            }
            if (selectedRows.Count == 1)
            {

                teacherId = Convert.ToInt64(selectedRows[0].Cells["colTeacherId"].Value);
            ...
            {
                MessageBoxManager.ShowErrorMessageBox(this, "Vui lòng chỉ chọn một giảng viên duy nhất!", "Thao Tác Sai");
                return;
            }
        }
